    ...
    this
      ...
      .validateFields((err, value) => {
        if (err) {
          ...
          console.log(value);
          // 发送请求 fetch
          fetch(`${config["r_s_url"]}?action=${this.state.action}&username=${value.s_username}&password=${value.s_password}&r_userName=${value.r_username}&r_password=${value.r_password}&r_confirmPassword=${value.r_confirmPassword}`, {method: "GET"}).then((response) => {
      ...
      ? (
        <Menu.Item key="logout">
          <Menu mode="horizontal">
            ...
            <SubMenu
                title={<Button type="primary" htmlType="button" >{this.state.userNickName}</Button>}>
                {/* <MenuItemGroup title="Item 1"> */}
                  <Menu.Item key="settings">个人中心</Menu.Item>
                  <Menu.Divider />
